...
/** When triggered, holds a high value for a specified time before going low
 * again */
struct PulseGenerator {
  ...
  /** Advances the state by `deltaTime`. Returns whether the pulse is in the
   * HIGH state. */
  ...
  void trigger(float triggerDuration) {
    // Keep the previous triggerDuration if the existing pulse would be held
    // longer than the currently requested one.

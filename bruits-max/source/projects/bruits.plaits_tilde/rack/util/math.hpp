...
/** Returns 1.f for positive numbers and -1.f for negative numbers (including
 * positive/negative zero) */
inline float sgn(float x) { return copysignf(1.0f, x); }
...
*/
inline void cmult(float *cr, float *ci, float ar, float ai, float br,
                  float bi) {
  *cr = ar * br - ai * bi;
  *ci = ar * bi + ai * br;
}
...
struct Vec {
  ...
  Vec neg() { return Vec(-x, -y); }
  Vec plus(Vec b) { return Vec(x + b.x, y + b.y); }
  Vec minus(Vec b) { return Vec(x - b.x, y - b.y); }
  Vec mult(float s) { return Vec(x * s, y * s); }
  Vec mult(Vec b) { return Vec(x * b.x, y * b.y); }
  Vec div(float s) { return Vec(x / s, y / s); }
  Vec div(Vec b) { return Vec(x / b.x, y / b.y); }
  float dot(Vec b) { return x * b.x + y * b.y; }
  float norm() { return hypotf(x, y); }
  Vec flip() { return Vec(y, x); }
  Vec min(Vec b) { return Vec(rack::min(x, b.x), rack::min(y, b.y)); }
  Vec max(Vec b) { return Vec(rack::max(x, b.x), rack::max(y, b.y)); }
  Vec round() { return Vec(roundf(x), roundf(y)); }
  Vec floor() { return Vec(floorf(x), floorf(y)); }
  Vec ceil() { return Vec(ceilf(x), ceilf(y)); }
  bool isEqual(Vec b) { return x == b.x && y == b.y; }
...
struct Rect {
  ...
  /** Returns whether this Rect contains an entire point, inclusive on the
   * top/left, non-inclusive on the bottom/right */
  bool contains(Vec v) {
    return pos.x <= v.x && v.x < pos.x + size.x && pos.y <= v.y &&
           v.y < pos.y + size.y;
  }
  ...
  bool isEqual(Rect r) { return pos.isEqual(r.pos) && size.isEqual(r.size); }
  Vec getCenter() { return pos.plus(size.mult(0.5f)); }
  Vec getTopRight() { return pos.plus(Vec(size.x, 0.0f)); }
  Vec getBottomLeft() { return pos.plus(Vec(0.0f, size.y)); }
  Vec getBottomRight() { return pos.plus(size); }
  ...
  Rect clamp(Rect bound) {
    ...
    r.size.x =
        rack::clamp(pos.x + size.x, bound.pos.x, bound.pos.x + bound.size.x) -
        r.pos.x;
    r.size.y =
        rack::clamp(pos.y + size.y, bound.pos.y, bound.pos.y + bound.size.y) -
        r.pos.y;
    return r;
  }
...
inline Vec Vec::clamp(Rect bound) {
  return Vec(rack::clamp(x, bound.pos.x, bound.pos.x + bound.size.x),
             rack::clamp(y, bound.pos.y, bound.pos.y + bound.size.y));
}

inline Vec Vec::clamp2(Rect bound) {
  return Vec(rack::clamp2(x, bound.pos.x, bound.pos.x + bound.size.x),
...
DEPRECATED inline void cmultf(float *cr, float *ci, float ar, float ai,
                              float br, float bi) {
  return cmult(cr, ci, ar, ai, br, bi);
}

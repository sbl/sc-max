...
// "ANTIDERIVATIVE ANTIALIASING FOR MEMORYLESS NONLINEARITIES" BY S. BILBAO ET
// AL. IEEE SPL, 2017
//
...
public:
	...
	float hardClipN2(float x) {
		...
		return oneTwelfth
			* (signum(x + 1.0f) * (x + 1.0f) * (x + 1.0f) * (x + 1.0f) - signum(x - 1.0f) * (x - 1.0f) * (x - 1.0f) * (x - 1.0f)
				  - 6.0f * x);
	}
...
class Wavefolder {
	...
	// THIS CLASS IMPLEMENTS A FOLDING FUNCTION, SOMETIMES KNOWN AS A MATHEMATICAL
	// FOLDER DUE TO ITS SHARP EDGES. THE SOUND PRODUCED IS SIMILAR TO THAT OF THE
	// BUCHLA 259'S TIMBRE SECTION.
